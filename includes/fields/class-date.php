...
class Field_Date {
    ...
    public function preview($config = []) {
        ...
        $preview .= "<input type='date' name='{$config['id']}' id='{$config['id']}' value='' min='1997-01-01' max='2030-12-31' disabled placeholder='" . (isset($config['placeholder']) ? $config['placeholder'] : '') . "'>";
        $preview .= "</div>";
        $preview .= "</div>";
        return $preview;
    }
