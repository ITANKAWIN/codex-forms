...
class Field_Nickname {
    ...
    public function preview($config = []) {
        ...
        $preview .= "<input type='text' name='nickname' id='{$config['id']}' disabled placeholder='" . (isset($config['placeholder']) ? $config['placeholder'] : '') . "' " . ($config['require'] == 'on' ? 'required' : '') . " value='" . (isset($config['value']) ? $config['value'] : '') . "'>";
        $preview .= "</div>";
        $preview .= "</div>";
        return $preview;
    }
