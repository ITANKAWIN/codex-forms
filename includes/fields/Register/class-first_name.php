...
class Field_First_Name {

    private $field_type = 'first_name';
    ...
    public function preview($config = []) {
        ...
        $preview .= "<input type='text' name='first_name' id='{$config['id']}' disabled placeholder='" . (isset($config['placeholder']) ? $config['placeholder'] : '') . "' " . ($config['require'] == 'on' ? 'required' : '') . " value='" . (isset($config['value']) ? $config['value'] : '') . "'>";
        $preview .= "</div>";
        $preview .= "</div>";
        return $preview;
    }
