...
class Field_Con_Password_User {

    private $field_type = 'con_password_user';
    ...
    public function preview($config = []) {
        ...
        $preview .= "<input type='password' name='con_password' id='{$config['id']}' disabled placeholder='" . (isset($config['placeholder']) ? $config['placeholder'] : '') . "' " . ($config['require'] == 'on' ? 'required' : '') . " value='" . (isset($config['value']) ? $config['value'] : '') . "'>";
        $preview .= "</div>";
        $preview .= "</div>";
        return $preview;
    }
