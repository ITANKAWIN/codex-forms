...
class Field_Con_Email_User {

    private $field_type = 'con_email_user';
    ...
    public function preview($config = []) {
        ...
        $preview .= "<input type='email' name='con_email' id='{$config['id']}' disabled placeholder='" . (isset($config['placeholder']) ? $config['placeholder'] : '') . "' " . ($config['require'] == 'on' ? 'required' : '') . ">";
        $preview .= "</div>";
        $preview .= "</div>";
        return $preview;
    }
