...
class Field_reset {
    ...
    public function preview($config = []) {
        ...
        $preview .= "<button type='reset' name='field_id[{$config['id']}]' id='{$config['id']}' class='ui secondary large button' disabled>" . (!empty($config['text']) ? $config['text'] : '') . "</button>";
        $preview .= "</div>";
        $preview .= "</form>";
        return $preview;
    }
